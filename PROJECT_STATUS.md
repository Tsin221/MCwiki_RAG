...
1. 公开部署前增加限流、请求预算、监控、HTTPS Cookie 和滥用防护；
2. 评估来源按页面进一步合并，减少同一 Wiki 页面多 chunk 带来的重复卡片；
3. 继续保留同一 18 题作为检索、提示词和模型变更的固定回归；
4. 只有出现“候选已召回但最终排序仍不足”的证据时，再评估 Reranker；
5. 公开服务边界稳定后，再评估受限联网的 Agentic RAG。
