...
完整链路质量基线已经生成，整体达到建议门槛。当前决定暂不接入 Reranker，先修复
版本号查询召回、证据反向解读和多子问题完整性这三个已定位问题。
...
- 平均正确性：1.80/2，平均完整性：1.80/2，平均证据忠实度：1.93/2；
- 期望来源引用率：14/15（93.33%），引用编号有效率：100%；
- 无答案可靠拒答率：3/3（100%）；
- 评测报告：`data/evaluation/answer_quality/REPORT-2026-07-30.md`。
...
1. 修复版本号查询的 BM25/标题召回，使 Java 版 1.21 页面进入候选；
2. 将灾厄巡逻队光照条件写反和苦力怕条件掉落遗漏加入固定回归；
3. 使用同一 18 题重新生成对比基线；
4. 只有候选已召回但排序仍不足时，再评估 Reranker；
5. 公开部署前增加限流、预算、监控、HTTPS Cookie 和滥用防护；
6. 评估来源按页面进一步合并，减少同一 Wiki 页面多 chunk 带来的重复卡片。
