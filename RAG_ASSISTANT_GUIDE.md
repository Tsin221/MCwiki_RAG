...
当前已形成可在本地浏览器使用的完整 RAG 问答链路。首份回答基线达到建议门槛；
版本召回失败、证据反向解读和回答不完整三个回归目标已经完成修复，同一 18 题新
基线的正确性、完整性和证据忠实度均为 2.00/2。下一阶段继续深化基础 RAG 工程，
不把限流、请求预算、监控、HTTPS Cookie 和滥用防护作为当前任务。

项目未来计划扩展为证据驱动的 Agentic RAG，但当前不实施。先使用固定回归集继续
夯实基础 RAG；未来方案、安全边界和实施前置条件见 `docs/ideas/agentic-rag.md`。
...
- 定向质量修复后的同一 18 题基线已完成；
- 修复后平均正确性、完整性和证据忠实度均为 2.00/2；
- 期望来源召回率与引用率均为 15/15；
- 引用编号有效率和无答案可靠拒答率均为 100%；
- 当前仍不接入 Reranker，下一阶段继续深化基础 RAG 工程；
- 同一 18 题固定为后续所有相关变更的回归门槛。
...
每次更换分块策略、Embedding 模型、检索或融合方法、上下文策略、Reranker、提示词
或 LLM 时，必须使用同一 18 题进行对比，生成新基线且不得覆盖历史结果。
...
当前唯一固定的下一步是维护同一 18 题回归门槛。先通过回归结果定位基础 RAG 的具体
瓶颈，再从上述方向选择改进项；基础 RAG 稳定后，才进入证据驱动的 Agentic RAG。
限流、请求预算、监控、HTTPS Cookie 和滥用防护留到项目明确需要公开部署时再评估。
