...
- **已完成**：15 个可回答问题与 3 个无答案问题的完整回答链路质量基线。

当前已形成可在本地浏览器使用的完整 RAG 问答链路。公开部署前仍需补充限流、
预算、监控和滥用防护。首份回答基线达到建议门槛，但保留了版本召回失败、证据
反向解读和回答不完整三个回归目标。
...
- Reranker：暂不接入；当前失败首先需要修复召回和生成，而不是重排。
...
4. RRF 合并两路结果；
5. 系统对证据去重并按数量和字符预算组成上下文；
6. LLM 根据问题和上下文生成回答；
7. 系统返回回答，并展示编号引用和来源。

当前链路尚未接入 Reranker。只有完整回答质量基线表明现有排序不足时，才在第 4 步
之后增加精排阶段。
...
- 15 个可回答问题与 3 个无答案问题的首份基线已完成；
- 平均正确性 1.80/2、完整性 1.80/2、证据忠实度 1.93/2；
- 引用编号有效率和无答案可靠拒答率均为 100%；
- 当前暂不接入 Reranker，先修复版本查询召回和已知回答问题。
