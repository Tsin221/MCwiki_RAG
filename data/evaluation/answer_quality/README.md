...
运行文件会记录数据集、系统提示词和实际生成参数的指纹。续跑时任一固定输入发生
变化都会被拒绝，避免把不同配置的回答混入同一基线。

## 固定回归门槛

现有 15 个可回答问题和 3 个无答案问题，共 18 个唯一案例，是基础 RAG 后续工程改动
的固定回归集。案例 ID、文件位置和参考基线固定在
`../regression_suite.json`（`mcwiki-rag-regression-v1`）中，并由自动化测试校验。
以下变更都必须重新运行同一 18 题并与最新修复后基线对比：

- 分块与元数据策略；
- Embedding 模型或向量检索配置；
- BM25、查询处理、融合或未来的 Reranker；
- 证据去重、上下文数量或字符预算；
- 系统提示词、回答参数或回答模型。

回归时必须保留失败问题和历史基线，不得通过删除问题、修改正确来源或覆盖旧结果来
提高表面指标。若确需修改评测集，应创建有明确版本说明的新评测集，同时保留旧版本，
不能把新旧案例混入同一次运行。
...
uv run --with pytest python -m pytest tests/test_evaluation.py
```
...
汇总时会对照同一组可回答与无答案数据集校验案例 ID；运行未完成、案例重复或混入
未知案例时不会生成汇总。
